...
- **Hero `--accent-bg` is theme-aware**: `accent.applyHsl` reads `documentElement.classList.contains('light')` and picks L=86%/low-saturation for light themes vs L=22%/medium-saturation for dark. The `wax:theme-changed` event (dispatched by `prefs.setTheme`) is listened to in `App.vue` to re-run `applyUserAccent` AND re-extract from the current track's cover (auto mode), keeping the hero in sync after a theme switch.
