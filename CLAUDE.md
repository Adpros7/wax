...
- `format.js` — `fmtDuration`, `debounce`, `gradientFromString` (hash-based color, fades to `var(--main)`), `eqHtml`, `YT_REGEX`, `isYoutubeUrl`, `isPlaylistUrl`, `isStreamId`. `onThumbError` swaps the `<img>`'s `src` to `/placeholder-cover.svg` (a neutral grey vinyl placeholder under `public/`) on failure; the loop guard checks `src.endsWith('placeholder-cover.svg')` rather than a sticky dataset flag so long-lived `<img>` elements (like the player thumb that gets a new `src` on every track change) keep behaving correctly. `onThumbLoad` is now a no-op kept for template-binding backward compat — server-side cover endpoint handles all variant fallback and grey-placeholder rejection.
